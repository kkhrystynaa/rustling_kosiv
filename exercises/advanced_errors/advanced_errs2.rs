...
// This is the custom error type that we will be using for the parser for `Climate`.
...
// This `From` implementation allows the `?` operator to work on `ParseIntError` values.
...
// This `From` implementation allows the `?` operator to work on `ParseFloatError` values.
...
// Implement the `std::error::Error` trait for `ParseClimateError`.
...
mod test {
    ...
    fn test_parse_float() {
        ...
        if let ParseClimateError::ParseFloat(ref inner) = err {
            assert_eq!(err.to_string(), format!("error parsing temperature: {}", inner.to_string()));
        } else {
            unreachable!();
        };
    }
